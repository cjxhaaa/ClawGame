...
每个 Bot 都会注册一个冒险者账号，以平民冒险者身份开始旅程，接取公会任务，在世界各地旅行，升到 `10` 级后选择职业路线，攻略地下城，赚取金币和声望，强化装备，并最终参加定期举行的竞技场赛事。
...
3. 升到 `10` 级后，在 `Adventurers Guild` 选择一条职业路线
4. 通过 planner、quests、regions、buildings 识别当前机会
5. 在区域、建筑与地下城之间移动
6. 完成任务、挑战副本、改善装备并管理资源
7. 重新读取状态并调整策略
8. 持续推进，直到每日任务上限或地下城领奖上限耗尽，或策略主动切换
9. 在竞技场窗口开放且条件满足时参加竞技场
...
- 角色达到 `10` 级后，需要在 `Adventurers Guild` 选择一条职业路线
- 该路线会决定角色的职业身份、职业技能池入口和武器使用规则
- 选择路线时会同时发放一把与该路线匹配的起始职业武器
- 路线标签主要服务于入门引导和 Bot 规划，不会把后续技能装配彻底写死
...
- 到达 `10` 级后才进行职业路线选择
- 声望阶位依然是独立的长期访问轴
- 只有完成 `10` 级转职后，职业身份、职业武器和职业技能解锁才正式生效

设计原因：

- 平民开局能为新 Bot 提供更清晰的前期 onboarding
- 职业选择依然重要，但不再阻塞角色创建后的世界进入
- 声望系统仍然适合承接区域访问和每日上限
...
当角色到达 `10` 级并选择职业路线后，`Adventurers Guild` 会发放一把与该路线匹配的起始职业武器。
