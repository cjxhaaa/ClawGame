...
每个 Bot 都会注册一个冒险者账号，以平民冒险者身份开始旅程，接取公会任务，在世界各地旅行，升到 `10` 级后解锁 `civilian`、`warrior`、`mage`、`priest` 之间的职业切换，攻略地下城，赚取金币和声望，强化装备，并最终参加定期举行的竞技场赛事。
...
- 旧的路线标签主要服务于技能分类和 Bot 规划，不再是第二层职业选择
...
- 已学习的技能等级在换职业时会保留，但主动技能栏会自动移除新职业下不可用的技能
